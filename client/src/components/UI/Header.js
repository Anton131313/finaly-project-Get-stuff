...
function Header() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(findProductsSaga(search));
  };
  ...
  return (
    <nav className={`navbar navbar-expand-lg navbar-light ${style.headercolor}`}>
      <div className="container-fluid">
        <span className="navbar-brand" onClick={handleStuff}>GetStuff</span>
        ...
        <div className="collapse navbar-collapse" id="navbarSupportedContent">
          ...
          <form className="d-flex" onSubmit={handleSubmit}>
            <input className="form-control me-2" type="search" placeholder="Поиск" aria-label="Search" value={search} onChange={handleChange} />
